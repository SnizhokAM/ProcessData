#Звіт по додатковій мітці імені користувача
...
def get_report():
    ...
    cur = conn.cursor()    
    cur.execute("SELECT date, time, chat_id, name, recipient FROM recipients WHERE name NOT LIKE '%(X)%';") #де (X) - додаткова мітка
    count_r = 0
    count_p = 0
    found = False
...
if __name__ == "__main__":
    with open("tmp\\report_xxx.txt", "w") as file:
        file.write(get_report())
    webbrowser.open("tmp\\report_xxx.txt")
